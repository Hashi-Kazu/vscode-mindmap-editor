...
### Fixed
- 本文ノードをダブルクリック／F2で編集開始した際にテキストが全選択状態になる不具合を修正。編集開始時はキャレットをテキスト末尾に配置するようにした。新規本文項目追加直後の自動編集（デフォルトテキスト全選択）は従来どおり維持（Issue #63 / R-13-05）。
...
### Changed
- 本文リスト項目（`- xxx`）のラベル表示から行数上限によるクランプ・クリップ・ホバー時ツールチップ全文表示を撤廃した。実際の折り返し行数・`<br>`改行数に応じてノード高さが上限なく伸長し、全文が常に表示される。幅制約（`BODY_MIN_W`〜`BODY_MAX_W`）と見出しノードの表示仕様（幅・高さ・2行クランプ・省略時ツールチップ）は変更なし。

## [2.24.0] - 2026-08-01

### Added
- 本文リスト項目（`- xxx`）のラベル表示で `<br>` / `<br/>` / `<br />`（大小文字ゆれ許容）を実際の改行として描画するようにした。編集用入力欄は従来どおり単一行で生の `<br>` を表示・編集し、編集中の Shift+Enter でカーソル位置に文字列 `<br>` を挿入できるようにした（既存 Enter は編集確定のまま変更なし）。Markdown上は1行内の文字列として保持され、パース・シリアライズ挙動は変わらない。見出しノードは対象外（R-22 新設）。
...
- 受け入れテスト仕様をv1.7.0（USDM v2.22.10対応）へ更新し、欠落していた7仕様IDのATと、未検証23件の自動／手動検証経路を追加した。未実施の受け入れテストはPASS扱いにしていない。
...
- Issue #53対応ではparser・serializer・Webviewの実行時コードおよび挙動を変更していない。
